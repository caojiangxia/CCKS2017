...
def do_(ans,pos,ok):
    ...
    if(ok==3):
        if(in_(ans[pos]) and jud!=3):
            if (pos - 2 >= 0 and (ans[pos + 1] == "n" or ans[pos + 1] == "nz") and (ans[pos - 1] == "x" or ans[pos - 1].startswith("m"))):
                if(ans[pos-1]=="x"):
                    if(ans[pos-2] in enti):
                        enti.remove(ans[pos-2])
                    enti.add(ans[pos - 2] + ans[pos])
                elif(in_(ans[pos - 2] + ans[pos])):
                    if (ans[pos - 2] in enti):
                        enti.remove(ans[pos - 2])
                    enti.add(ans[pos - 2] + ans[pos])
            else:
                if(len(ans[pos])>1 and not_have(ans[pos])):
                    enti.add(ans[pos])
    elif(ok==1) :
        if(jud==1):
            YES=1
            ...
            if(pos-2>=0 and (ans[pos+1]=="n" or ans[pos+1]=="nz") and (ans[pos-1]=="x" or ans[pos-1].startswith("m") or ans[pos-1]=="ntc")):
                if (ans[pos - 1] == "x"):
                    if (ans[pos - 2] in enti):
                        enti.remove(ans[pos - 2])
                    enti.add(ans[pos - 2] + ans[pos])
