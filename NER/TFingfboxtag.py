...
def _calculate(corpus,candidate):
    n=len(corpus)
    m=len(corpus[0])
...
def getvector(s,res):
    ...
    for word in s:
        if res.get(word) is not None:
            ...
            ret.append(int(0))
    return ret
def extend(word_line,res,title):
    if allMention.get(res) is not None:
        if allMention[res].get(title) is not None:
            info=allMention[res][title]["infonbox"]
                ...
                if (line[i] == '|' or line[i]=="\n"):
                    ...
                    if (allMention.get(res) is not None):
                        ...
                        for title in allMention[res]:
                            ...
                            word_line = extend(word_line, title)
                        ...
                        if (wordnum.get(res) is None):
                            wordnum[res] = {}
                        word_line = mention
                        word_line=extend(word_line,res)
                        for word in word_line:
                            allword.add(word)
                        D = {}  # 需要单独计算
                        for word in word_line:
                        ...
                        if(len(candidatetitle)):
                            queue.append(candidatetitle[_calculate(candidate,candidatetitle)])
                        else :
                            queue.append("nil")
                    else:
                        queue.append("nil")
