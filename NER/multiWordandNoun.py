...
def do_(ans,pos,ok):
    ...
    if(ok==3):
        if(in_(ans[pos]) and jud!=3):
            if (pos - 2 >= 0 and (ans[pos + 1] == "n" or ans[pos + 1] == "nz") and (
                 ans[pos - 1] == "x" or ans[pos - 1] == "m") and in_(ans[pos - 2] + ans[pos])):
        ...
        if(jud==1):
            ...
            if(pos-2>=0 and (ans[pos+1]=="n" or ans[pos+1]=="nz") and (ans[pos-1]=="x" or ans[pos-1]=="m" or ans[pos-1]=="ntc") and in_(ans[pos-2]+ans[pos])):
                enti.add(ans[pos-2]+ans[pos])
                YES=1
            elif(in_(ans[pos])):
                if (len(ans[pos])>1):
            ...
            if (pos - 2 >= 0 and ans[pos - 1]=="x" and len(ans[pos - 2])>2):
                if(in_(ans[pos-2])):
                    enti.add(ans[pos-2]);
    pos+=2
    if(ook!=-1):
...
def div_to_excle(ans,A,B,C):  #     ans-细粒度，A-正确答案，B-baseline，C-test。
