...
def getvector(s,res):
    ...
    return ret
def find_extend(res):
    global allindex,alldata
    word_line=[]
    RES=""
    for i in range(len(res)):
        for j in range(i+1,len(res)):
            RES=res[i:j]
            if(allindex.get(RES) is not None):
                for k in allindex[RES]:
                    word_line.append(change(alldata[k]))
    return word_line
def get_ans(name):
    fr = open(name, "r")
    fw = open("第三列2.txt", "w")
                ...
                if (line[i] == '|' or line[i]=="\n"):
                    ...
                    if (allMention.get(res) is not None):
                        ...
                        if (wordnum.get(res) is None):
                            wordnum[res] = {}
                        word_line=find_extend(res)
                        word_line.append(mention)
                        D = {}  # 需要单独计算
                        for i in range(len(word_line)):
                            for word in word_line[i]:
                                allword.add(word)
        ...
        for res in line.split():
            ...
            for i in range(len(res)):
                if (res[i] == '['):
                    ...
                    if(allindex.get(ans) is None):
                        allindex[ans]=[]
                    allindex[ans].append(cnt)
                    break
                else :
                    ans=ans+res[i]
        cnt+=1
