...
def _calculate(corpus,value,title):
    n=len(corpus)
    m=len(corpus[0])
...
def cal_primary(res,title):
    #if(dic.get(res) is not None and (dic[res].startswith("ns") or dic[res].startswith("nt") ) and allMention[res].get("primary") is not None and allMention[res]["primary"]==title):
    if (allMention[res].get("primary") is not None and allMention[res]["primary"] == title):
        if(allMention[res]["primary"] == title):
            return float(1)
    return float(0.3)
                ...
                if (line[i] == '|' or line[i]=="\n"):
                    ...
                    if (allMention.get(res) is not None):
                        ...
                        for title in allMention[res]:
                            ...
                            R=1
                            if (allMention[res].get("primary") is not None):
                                R*=cal_primary(res,title)
                            # elif(dic.get(res) is not None and (dic[res].startswith("ns") or dic[res].startswith("nr") or dic[res].startswith("nt"))):
                            #     R*=cal_type(res, title)
                            else :
                                R*=distance(title, res)
                        ...
                        if(len(candidatetitle)):
                            queue.append(candidatetitle[_calculate(candidate,value,candidatetitle)])
                        else :
                            queue.append("nil")
                    else:
                        queue.append("nil")
