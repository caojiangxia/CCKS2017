...
def change2(line,step):
    res=change(line)
    ret=[]
    beg=int(0)
    for i in range(len(res)):
        if(res[i].startswith(step)):
            for j in range(max(beg,max(int(0),i-wide)),min(i+wide+1,len(res))):
                ret.append(res[j])
            beg=min(i+wide+1,len(res))
    return ret
                    ...
                    if (allMention.get(res) is not None):
                        ...
                        for title in allMention[res]:
                            if (allMentionAbs.get(res) is None or allMentionAbs[res].get(title) is None):
                                continue
                            word_line=change2(allMentionAbs[res][title],res)#所有单词的
