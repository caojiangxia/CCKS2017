...
import jellyfish
import numpy as np
from EntityLink import entLinkRead
...
def jaro_distance(s1, s2):
    return float(jellyfish.jaro_distance(s1, s2))

def dam_distance(s1, s2):
    return float(1 - (jellyfish.damerau_levenshtein_distance(s1, s2) * 1.0 / max(len(s1), len(s2))))

def _calculateCos(corpus, dis):
    ...
    diss = np.asarray(dis)
    diss = diss.reshape((1, diss.shape[0]))
    maxsim = (diss*sims).max(axis=1)[0]
    maxidx = (diss*sims).argmax(axis=1)[0]
    return int(maxidx)
...
def get_ans(name):
    ...
    for line in fr:
                    ...
                    if (allMention.get(res) is not None):
                        for title in allMention[res]:
                            ...
                            if (allMentionAbs.get(res) is None or allMentionAbs[res].get(title) is None):
                                continue
                            T.append(title)
                            Anchor[title]=allMention[res][title]["anchor"]
                            specialword[title]=change(allMentionAbs[res][title])
                    ret[2].append(T)
                    enti.append(res)
                    res = ""
                else:
        ...
        ret[3]=specialword
        line.strip()
        queue=entLinkRead(ret)
        # fww = open('LIST.pkl', 'wb')
        # pickle.dump(ret, fww, 2)
        # fww.close()
...
0.3130870381966187  NO-distance
0.3147434597444737  distance
0.3094706955992699  jaro_distance
0.3147434597444737  dam_distance
'''
